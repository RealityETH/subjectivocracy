...
contract RealityETH_v3_0 is BalanceHolder, IRealityETH {
    ...
    modifier bondMustDoubleAndMatchMinimum(bytes32 question_id) {
        ...
        if (current_bond == 0) {
            require(
                msg.value >= (questions[question_id].min_bond),
                "bond must exceed the minimum"
            );
        } else {
            require(
                msg.value >= (current_bond * 2),
    ...
    function fundAnswerBounty(
        bytes32 question_id
    ) external payable stateOpen(question_id) {
        ...
        previousBondMustNotBeatMaxPrevious(question_id, max_previous)
    {
        bytes32 commitment_id = keccak256(
            abi.encodePacked(question_id, answer_hash, msg.value)
        );
        ...
        )
    {
        ...
        bytes32 question_id = askQuestionWithMinBond(
            template_id,
            question,
            arbitrator,
            timeout,
    ...
    function getEarliestAnswerFromSuppliedHistoryOrRevert(
        ...
        uint256[] memory bonds,
        bytes32[] memory answers
    ) external view stateAny returns (bytes32, uint256) {
        // Go through the history reverting if any of it is wrong
        bool is_commitment;
        bytes32 last_history_hash = questions[question_id].history_hash;
        uint256 i;
        for (i = 0; i < history_hashes.length; i++) {
            is_commitment = _verifyHistoryInputOrRevert(
                last_history_hash,
                history_hashes[i],
                answers[i],
                bonds[i],
        ...
        if (is_commitment) {
            require(
                commitments[answer_or_commitment_id].is_revealed,
