...
contract ForkManager is Arbitrator, IERC20, ERC20 {
    ...
    uint32 public constant REALITY_ETH_TIMEOUT = 604800; // 1 week

    // The standard reality.eth delimiter for questions with multiple arguments
    string public constant QUESTION_DELIM = "\u241f";
    ...
    uint256 public constant POST_FORK_FREEZE_TIMEOUT = 604800;
    ...
    function init(
        address payable _parentForkManager,
        address _realityETH,
        address _bridgeToL2,
        bool _has_governance_freeze,
        uint256 _parentSupply,
        address payable _libForkManager,
        address _libForkableRealityETH,
        address _libBridgeToL2,
        address _initialRecipient,
        uint256 _initialSupply
    ) external {
        require(
            address(libForkManager) == address(0),
        ...
        require(
            address(_realityETH) != address(0),
            "RealityETH address must be supplied"
        );
        require(
            address(_bridgeToL2) != address(0),
            "Bridge address must be supplied"
        );
        ...
        if (_has_governance_freeze) {
            initialGovernanceFreezeTimeout =
                block.timestamp +
                POST_FORK_FREEZE_TIMEOUT;
        }
    ...
    function importProposition(
        ...
        propositions[question_id] = ArbitratorProposition(
            proposition_type,
    ...
    function effectiveTotalSupply() internal view returns (uint256) {
        if (
            forkedFromParentTs == 0 ||
            (block.timestamp - forkedFromParentTs > FORK_TIME_SECS)
        ) {
            return totalSupply;
        } else {
    ...
    function mint(address _to, uint256 _amount) external {
        require(
            msg.sender == address(parentForkManager),
            "Only our parent can mint tokens"
        );
    ...
    function deployFork(
        ...
        if (yes_or_no) {
            require(
                address(childForkManager1) == address(0),
                "Already migrated"
            );
            result = bytes32(uint256(1));
        } else {
            require(
                address(childForkManager2) == address(0),
                "Already migrated"
            );
        ...
        require(
            ...
            "Wrong parameters supplied for last answer"
        );
        ...
        bool upgrade_bridge = (yes_or_no &&
            (propositions[fork_question_id].proposition_type ==
                PropositionType.UPGRADE_BRIDGE));
        if (upgrade_bridge) {
        ...
        ZKBridgeToL2 newBridgeToL2 = ZKBridgeToL2(
            _deployProxy(bridgeLibForThisDeployment)
        );
        ...
        */
        ...
        ForkableRealityETH_ERC20 newRealityETH = ForkableRealityETH_ERC20(
            _deployProxy(libForkableRealityETH)
        );
        newRealityETH.init(
            IERC20(newFm),
            address(realityETH),
        ...
        newFm.importProposition(
            fork_question_id,
            propositions[fork_question_id].proposition_type,
            propositions[fork_question_id].whitelist_arbitrator,
            propositions[fork_question_id].arbitrator,
            propositions[fork_question_id].bridge
        );

        newFm.init(
            payable(address(this)),
            address(newRealityETH),
            address(newBridgeToL2),
            (numGovernanceFreezes > 0),
            supplyAtFork,
            libForkManager,
            libForkableRealityETH,
            libBridgeToL2,
            address(0),
            0
        );
    ...
    function requestArbitrationByFork(
        bytes32 question_id,
    ...
    function isWinner() public view returns (bool) {
        ...
        ForkManager parentReplacement = parentForkManager
            .replacedByForkManager();
    ...
    function isLoser() public view returns (bool) {
        if (address(parentForkManager) == address(0x0)) {
            // Genesis fork manager
            return false;
        }
    ...
    function disputeFee(bytes32) public view returns (uint256) {
    ...
    function beginAddArbitratorToWhitelist(
        address whitelist_arbitrator,
        address arbitrator_to_add
    ) external {
        string memory question = _toString(
            abi.encodePacked(
                whitelist_arbitrator,
                QUESTION_DELIM,
                arbitrator_to_add
        ...
        bytes32 question_id = realityETH.askQuestion(
            TEMPLATE_ID_ADD_ARBITRATOR,
            question,
            address(this),
            REALITY_ETH_TIMEOUT,
        ...
        require(
            propositions[question_id].proposition_type == PropositionType.NONE,
    ...
    function beginRemoveArbitratorFromWhitelist(
        address whitelist_arbitrator,
        ...
        string memory question = _toString(
            abi.encodePacked(
                whitelist_arbitrator,
                QUESTION_DELIM,
                arbitrator_to_remove
            )
        );
        bytes32 question_id = realityETH.askQuestion(
            TEMPLATE_ID_REMOVE_ARBITRATOR,
            question,
            address(this),
            REALITY_ETH_TIMEOUT,
            uint32(block.timestamp),
    ...
    function beginUpgradeBridge(address new_bridge) external {
        string memory question = _toString(abi.encodePacked(new_bridge));
        bytes32 question_id = realityETH.askQuestion(
            TEMPLATE_ID_BRIDGE_UPGRADE,
            question,
            address(this),
            REALITY_ETH_TIMEOUT,
            uint32(block.timestamp),
    ...
    function _verifyMinimumBondPosted(
        bytes32 question_id,
        uint256 minimum_bond
    ) internal view {
        require(
            !realityETH.isFinalized(question_id),
            "Question is already finalized, execute instead"
        );
        require(
            realityETH.getBestAnswer(question_id) == bytes32(uint256(1)),
            "Current answer is not 1"
        );
        require(
            realityETH.getBond(question_id) >= minimum_bond,
            "Bond not high enough"
        );
    }

    function clearFailedGovernanceProposal(bytes32 question_id) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
            "Proposition not found or wrong type"
        );
        require(
            propositions[question_id].proposition_type ==
                PropositionType.UPGRADE_BRIDGE,
            "Not a bridge upgrade proposition"
        );

        require(
            realityETH.resultFor(question_id) != bytes32(uint256(1)),
            "Proposition passed"
        );
    ...
    function executeBridgeUpgrade(bytes32 question_id) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
            "Proposition not recognized"
        );
        require(
            propositions[question_id].proposition_type ==
                PropositionType.UPGRADE_BRIDGE,
    ...
    function numTokensRequiredToFreezeBridges() public view returns (uint256) {
        return (effectiveTotalSupply() / 100) * PERCENT_TO_FREEZE;
    }

    function freezeBridges(bytes32 question_id) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
            "Proposition not recognized"
        );
    ...
    function executeAddArbitratorToWhitelist(bytes32 question_id) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
            "Proposition not recognized"
        );
        require(
            propositions[question_id].proposition_type ==
                PropositionType.ADD_ARBITRATOR,
            "Not an add arbitrator proposition"
        );
        address whitelist_arbitrator = propositions[question_id]
            .whitelist_arbitrator;
        address arbitrator_to_add = propositions[question_id].arbitrator;

        require(
            whitelist_arbitrator != address(0x0),
            "Proposition not recognized"
        );
        ...
        bytes memory data = abi.encodeWithSelector(
            WhitelistArbitrator(whitelist_arbitrator).addArbitrator.selector,
            arbitrator_to_add
        );
        bridgeToL2.requestExecute(
            whitelist_arbitrator,
            data,
            0,
            Operations.QueueType.Deque,
            Operations.OpTree.Rollup
        );
    ...
    function freezeArbitratorOnWhitelist(bytes32 question_id) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
            "Proposition not recognized"
        );
        require(
            propositions[question_id].proposition_type ==
                PropositionType.REMOVE_ARBITRATOR,
            "Not a remove arbitrator proposition"
        );
        ...
        bytes memory data = abi.encodeWithSelector(
            WhitelistArbitrator(arbitrator_to_remove).freezeArbitrator.selector,
            arbitrator_to_remove
        );
        bridgeToL2.requestExecute(
            whitelist_arbitrator,
    ...
    function executeRemoveArbitratorFromWhitelist(
        bytes32 question_id
    ) external {
        require(
            propositions[question_id].proposition_type != PropositionType.NONE,
        ...
        bytes memory data = abi.encodeWithSelector(
            WhitelistArbitrator(whitelist_arbitrator).removeArbitrator.selector,
            arbitrator_to_remove
        );
        bridgeToL2.requestExecute(
            whitelist_arbitrator,
    ...
    function executeUnfreezeArbitratorOnWhitelist(
        bytes32 question_id
    ) external {
        require(
            propositions[question_id].proposition_type ==
        ...
        require(
            realityETH.resultFor(question_id) == bytes32(uint256(0)),
            "Proposition passed"
        );

        bytes memory data = abi.encodeWithSelector(
            WhitelistArbitrator(whitelist_arbitrator)
                .unfreezeArbitrator
                .selector,
            arbitrator_to_remove
        );
    ...
    function requiredBridges()
        external
        view
        returns (address bridge1, address bridge2)
    {
        ...
        if (
            initialGovernanceFreezeTimeout > 0 &&
            block.timestamp < initialGovernanceFreezeTimeout
        ) {
            return (address(0), address(0));
        }
        ...
        if (isForkingResolved()) {
            ...
            return (
                address(childForkManager1.bridgeToL2()),
                address(childForkManager2.bridgeToL2())
            );
        }
    }
    ...
    function bid(uint8 _bid, uint256 _amount) external {
        require(address(auction) != address(0), "Auction not under way");
        require(
            balanceOf[msg.sender] >= _amount,
            "Balance lower than bid amount"
        );
        balanceOf[msg.sender] = balanceOf[msg.sender] - _amount;
        auction.bid(msg.sender, _bid, _amount);
    }

    function settleBid(uint256 bid_id, bool yes_or_no) external {
        require(address(auction) != address(0), "Auction not under way");
        (address payee, uint256 due) = auction.clearAndReturnPayout(
            bid_id,
            yes_or_no
        );
        if (yes_or_no) {
