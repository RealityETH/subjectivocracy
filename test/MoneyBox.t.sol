...
contract MoneyBoxTest is Test {
    ...
    function testCreatorCanTakeTokens() public {
        ...
        address calculatedAddress = exampleMoneyBoxUser
            .calculateMoneyBoxAddress(user1, salt, address(token));
    ...
    function testAddressCalculation() public {
        ...
        ExampleMoneyBoxUser exampleMoneyBoxUser = new ExampleMoneyBoxUser();
        address calculatedAddress = exampleMoneyBoxUser
            .calculateMoneyBoxAddress(user2, salt, address(token));
        assertEq(calculatedAddress, address(moneyBox));

        address calculatedAddress2 = exampleMoneyBoxUser
            .calculateMoneyBoxAddress(user1, salt, address(token));
        assertNotEq(calculatedAddress2, address(moneyBox));

        address calculatedAddress3 = exampleMoneyBoxUser
            .calculateMoneyBoxAddress(
                user2,
                bytes32("0xee00bb"),
                address(token)
            );
        ...
        address calculatedAddress4 = exampleMoneyBoxUser
            .calculateMoneyBoxAddress(user2, salt, address(0xee00bb));
