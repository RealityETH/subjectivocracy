...
contract RealityETHTest is Test {
    ...
    bytes32 internal constant BYTES32_YES = bytes32(uint256(1));
    bytes32 internal constant BYTES32_NO = bytes32(uint256(0));
    ...
    function _logsToHistory(Vm.Log[] memory logs) internal {
        ...
        */
        ...
        bytes32 logNewAnswerSignature = keccak256(
    ...
    function setUp() public {
        ...
        uint256 bond1 = 1000;
        vm.prank(user1);
        realityEth.submitAnswerCommitment{value: bond1}(
            q1,
            answerHash1,
            0,
            user1
        ...
        uint256 bond2 = 5000;
        vm.prank(user2);
        realityEth.submitAnswerCommitment{value: bond2}(
            q1,
            answerHash2,
            0,
            user2
        ...
        uint256 bond3 = 10000;
        vm.prank(user1);
        realityEth.submitAnswerCommitment{value: bond3}(
            q1,
            answerHash3,
            0,
            user1
    ...
    function _checkSuppliedHistory(
        bytes32 expectedAnswer,
        uint256 expectedBond
    ) internal {
        (bytes32 finalAnswer, uint256 finalBond) = realityEth
    ...
    function testGetEarliestAnswerFromSuppliedHistoryOrRevertWrongHashReverts()
        public
    {
        // Make one of the history hashes wrong
        historyHashes[2] = bytes32(0);
        vm.expectRevert(
            "History input provided did not match the expected hash"
        );
        realityEth.getEarliestAnswerFromSuppliedHistoryOrRevert(
            q1,
            historyHashes,
